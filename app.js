...
postButton.addEventListener('click', () => {
    ...
    if (content !== '' || media) {
        const post = {
            ...
            dislikes: 0,
            comments: [],
            timestamp: new Date().toLocaleString() // Date and time of post
        };
        savePost(post);
        postContent.value = '';
        postMedia.value = '';
        ...
        if (post.username === currentUser || userFollows.includes(post.username)) {
            ...
            postItem.innerHTML = `
                ...
                <div class="comments" data-index="${index}">
                    ${post.comments.map(comment => `<p>${comment.content} <small>(${comment.timestamp})</small></p>`).join('')}
                </div>
            `;
...
function addComment(index, comment) {
    ...
    const newComment = {
        content: comment,
        timestamp: new Date().toLocaleString() // Date and time of comment
    };
